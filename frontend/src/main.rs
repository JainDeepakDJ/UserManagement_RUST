        ...
        Callback::from(move |_|{
            ...
            spawn_local(async move {
                match Request::get("http://127.0.0.1:8000/api/users").send().await {
                    ...
                    _ => message.set("Failed to fetch the users".into()),
                }
            });
        })
    };
...
let create_user = {
    ...
    Callback::from(move |_| {
        ...
        match response {
            Ok(resp) if resp.ok() => {
                message.set("User Created Succesfully".into());
                get_users.emit(());
            }
    ...
    Callback::from(move |_| {
        ...
        if let Some (id) = editing_user_id {
            spawn_local(async move {
                ...
                match response {
                    Ok(resp) if resp.ok() => {
                        message.set("User Updated Succesfully".into());
                        get_users.emit(());
                    }
...
let delete_user = {
    ...
    Callback::from(move |id: i32| {
        ...
        spawn_local(async move {
            ...
            match response {
                Ok(resp) if resp.ok() => {
                    message.set("User Deleted Succesfully".into());
                    get_users.emit(());
                }
...
let edit_user = {
    ...
    Callback::from(move |id: i32| {
        if let Some(user) = users.iter().find(|u| u.id == id){
            user_state.set((user.name.clone(), user.email.clone(), Some(user.id)));
        }
    })
};
...
html! {
    ...
    <button onclick = {if user_state.2.is_some() {update_user.clone()} else {create_user.clone()}}
    ...
    <button onclick = {get_users.reform(|_|())}
            ...
            {for (*users).iter().map(|user|{
                ...
                html!{
                    <li class = "mb-2">
                        ...
                        <button onclick = {delete_user.clone().reform(move |_| user_id)}
